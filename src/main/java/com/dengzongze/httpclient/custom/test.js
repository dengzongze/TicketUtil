    ...
    jQuery.extend({
        initLoginForm: function () {
            ...
            $.ajax({
                ...
                dataType: "json",
                xhrFields: {withCredentials: true},
                data: {answer: d, login_site: "E", rand: f},
                async: false,
                success: function (i) {
                    if (i.result_code == "4") {
                        e = true;
            ...
            TouClick.ready(function () {
                ...
                g.css({"float": "left"});
                var j = g.parent();
                j.css({marginTop: "0px", height: "auto", "padding-left": "0", zoom: "1"});
                var m = j.parent();
                var h = $("<ul></ul>");
                h.css({"float": "left", width: "147px"});
                m.css({"float": "left", width: "510px", borderLeft: "1px solid #bbb"});
                m.find("input").css({width: "309px"});
                h.insertBefore(m);
                m.children(".txt").css({display: "none"});
                m.children(".zc").eq(0).children().appendTo(j);
                m.children(".zc").appendTo(h);
                m.children(".error").css({position: "absolute"}).wrap($("<div></div>").css({height: "12px"}));
                h.children(".zc").css({
                    paddingLeft: "0",
                    marginTop: "0px",
                    height: "auto",
                ...
                m.parent().css({height: "340px"}).parent().next().css({height: "360px"}).find("li").css({paddingTop: "28px"});
            ...
            $.ajax({
                type: "POST", url: ctx + passport_authclient, data: {tk: d}, datatype: "json", success: function (e) {
                    if (e.result_code == 0) {
